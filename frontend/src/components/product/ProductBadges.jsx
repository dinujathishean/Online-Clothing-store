...
const styles = {
  ...
  oos: 'bg-neutral-400 text-neutral-900',
};
  ...
  return (
    <div className={`flex flex-wrap gap-1 ${className}`}>
      {badges.map((b) => (
        <span
          key={`${b.type}-${b.label}`}
          className={`rounded px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${styles[b.type] || 'bg-neutral-700 text-white'}`}
        >
          {b.label}
        </span>
      ))}
